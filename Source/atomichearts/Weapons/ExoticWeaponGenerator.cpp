...
{
    return GenerateExotic(FMath::RandRange(0, 99), Slot);
}
...
FLinearColor UExoticWeaponGenerator::GetTierColor(EExoticRarityTier Tier)
{
    switch (Tier)
    {
    case EExoticRarityTier::Common:    return FLinearColor(0.9f, 0.9f, 0.9f, 1.0f);
    case EExoticRarityTier::Rare:      return FLinearColor(0.3f, 0.5f, 1.0f, 1.0f);
    case EExoticRarityTier::Legendary: return FLinearColor(0.8f, 0.2f, 1.0f, 1.0f);
    case EExoticRarityTier::Mythic:    return FLinearColor(1.0f, 0.85f, 0.0f, 1.0f);
...
int32 UExoticWeaponGenerator::ComputeSeed(int32 ExoticIndex, EWeaponSlot Slot)
{
    ...
    for (int32 i = 0; i < 3; ++i) { Hash ^= Data[i]; Hash *= 16777619u; }
    ...
    int32& OutTraitCount)
{
    ...
    if (ValidTraits.IsEmpty()) { OutPrimary = EExoticTrait::None; OutSecondary = EExoticTrait::None; OutTraitCount = 0; return; }
    ...
    if (OutTraitCount == 2)
    {
        ...
        OutSecondary = SecondarySeed != PrimaryIdx ? static_cast<EExoticTrait>(SecondarySeed) : EExoticTrait::None;
    }
    else OutSecondary = EExoticTrait::None;
}
...
// ============================================================
// PERK GENERATION (60 perk pool)
// ============================================================

static const TArray<FString> GExoticPerkNames = {
...
static const TArray<FLinearColor> GPerkColors = {
    ...
    EExoticRarityTier Tier, FRandomStream& Stream)
{
    ...
    {
        const int32 PerkSeed = (Seed * 13 + i * 7) % GExoticPerkNames.Num();
        FWeaponPerk Perk;
        Perk.PerkName = GExoticPerkNames[PerkSeed];
        Perk.Description = FString::Printf(TEXT("Rank %d exotic perk"), (PerkSeed % 3) + 1);
        Perk.PerkRank = (PerkSeed % 3) + 1;
        Perk.PerkColor = GPerkColors[i % GPerkColors.Num()];
        OutPerks.Add(Perk);
    }
}
...
{
    const float TierMult = 1.0f + static_cast<float>(Tier) * 0.3f;
    ...
    if (Slot == EWeaponSlot::Heavy_RL || Slot == EWeaponSlot::Heavy_GL)
        OutStats.FireRate = 1.5f + Stream.FRand() * 0.5f;
    else if (Slot == EWeaponSlot::Secondary_SG || Slot == EWeaponSlot::Heavy_SW)
        OutStats.FireRate = 1.0f + Stream.FRand() * 0.8f;
    else
